...
const OrderListCard = ({ pro, refetch }) => {
  ...
  const handleOrderDelete = (id) => {
    ...
    if (agree) {
      fetch(`https://e-commerce-server-three.vercel.app/order/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
    ...
    <div className="card card-side bg-base-100 shadow-xl">
      ...
      <div className="card-body">
        ...
        <p className="text-xl mt-2 ">
          Price: <strong className="text-blue-600">{price}$</strong>
        </p>
        <div className="card-actions justify-end">
          <button
            onClick={() => handleOrderDelete(_id)}
            className=" border-none btn  bg-red-500 hover:bg-red-600 "
          >
            Delete
          </button>
        </div>
