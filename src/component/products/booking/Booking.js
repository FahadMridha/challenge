...
const Booking = () => {
  ...
  const handleCart = () => {
    ...
    fetch(`https://e-commerce-server-three.vercel.app/order`, {
      method: "POST",
      headers: {
      ...
      <div className="flex justify-center items-center p-5  lg:py-48 dark:bg-black">
        <div className="p-8 max-w-sm  rounded-lg border border-gray-200 shadow-md dark:bg-black bg-green-400 ">
          ...
          <p className="mb-1 text-sm font-normal  text-white">
            Thank You for booking the {name}.
          </p>
          <p className="mb-2">
          ...
          <p
            onClick={handleCart}
            className="inline-flex items-center py-2  px-3 text-lg font-medium text-center text-white hover:bg-orange-500  rounded-lg bg-blue-600 focus:ring-4 focus:outline-none  focus:ring-blue-800"
          >
            Confirm
          </p>

          <Link to="/products">
            <button className="ml-36 border-none btn  bg-red-500 hover:bg-red-600 ">
              Cancel
            </button>
          </Link>
        </div>
